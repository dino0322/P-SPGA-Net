...
#from .models.p_module import P_Module
...
class EfficientNetV2ForImageClassification_v2(nn.Module):
    ...
    def __init__(self, num_labels=7, img_size=224, patch_size=16, hidden_dim=512,
       ...
       #AttentionEnsemble
        ...
        self.attention_channels = num_features
        self.ensemble = AttentionEnsemble(self.attention_channels)
       ...
       #     self.p_module.train()
        feature_dim_fc = self.attention_channels
    ...
    def forward(self, images, labels=None):
        ...
        # Reshape for CBAM (add spatial dimensions)
        x = features.unsqueeze(-1).unsqueeze(-1)
        # CBAM
        x = self.ensemble(x)
        # SAFE module
       # x = self.p_module(x)
