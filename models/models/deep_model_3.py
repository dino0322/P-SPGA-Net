...
"""
This is <proposal 3>
Using CBAM & Triplet Attention & Coordinate Attention 
As an Ensenble Method

[How it works]
input -> {(CBAM / Triplet / Coordinate) - concat}
-> conv(1x1) -> residual(keep original imformation) -> out 

[Multi-Attention Structure]
- CBAM: channel & spatial attention
- Triplet: gives ChannelGateH(channel-height), ChannelGateW(channel-width), SpatialGate(height-width) 
           attention and average those 3 feature sets
- Coordinate: directional position attention based on long-range dependency
              keeps H/W 
"""
...
class AttentionEnsemble(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.cbam = CBAMBlock(channels)                                    #1cbam
        self.triplet = TripletAttention(gate_channels=channels)    #2triplet import
        self.coord = CoordAtt(channels)                  #3coordinate attention import
    ...
    def forward(self, x):
        ...
        triplet_out = self.triplet(x)
        coord_out = self.coord(x)

        out = torch.cat([cbam_out, triplet_out, coord_out], dim=1)
        out = self.fuse(out)
