...
class EfficientNetV2SAFE_Ablation(nn.Module):  #modify later
    ...
    def __init__(self, num_labels=2, img_size=224, patch_size=16, hidden_dim=512,
        ...
        if safe_type == 'none':
            ...
            self.p_module = SAFEModule_GELU(num_features, out_channels=64)
        elif safe_type == 'elu':
            self.p_module = SAFEModule_ELU(num_features, out_channels=64)
        elif safe_type == 'add':
            self.p_module = SAFEModule_DualPath_Add(num_features, out_channels=64)
        elif safe_type in ['multiply', 'full']:
            self.p_module = SAFEModule_DualPath_Multiply(num_features, out_channels=64)
        else:
            raise ValueError(f"Unknown safe_type: {safe_type}. Use 'none', 'gelu', 'elu', 'add', or 'multiply'/'full'")
        
        if self.p_module is not None:
...
class RepNeXtLiteBlock(nn.Module):
    def __init__(self, channels, expansion=2, act_layer=nn.SiLU):
        super().__init__()

        # distribute division by 4
        assert channels % 4 == 0, "channels must be divisible by 4"
        c = channels // 4

        # transform to diff scale
        self.branch_i = nn.Identity()
        self.branch_s = nn.Conv2d(c, c, kernel_size=3, padding=1, groups=c, bias=False)
        self.branch_m = nn.Conv2d(c, c, kernel_size=5, padding=2, groups=c, bias=False)
        self.branch_l = nn.Conv2d(c, c, kernel_size=5, padding=4, dilation=2, groups=c, bias=False)
...
class EfficientNetV2ForImageClassification_v2(nn.Module):
    ...
    def __init__(self, num_labels=7, img_size=224, patch_size=16, hidden_dim=512,
        ...
        # CBAM
        from models.models.deep_model import CBAMBlock
        ...
        #RepNeXt
        self.repnext_block = RepNeXtLiteBlock(channels=self.backbone.final_stage_channel, expansion=2)
        ...
        #    feature_dim_fc = self.cbam_channels
        #    feature_dim_fc = x.shape[1]

        with torch.no_grad():
            dummy = torch.zeros(1, self.cbam_channels, 1, 1)
            x = self.cbam(dummy)
            x = self.safe_module(x)
            x = torch.nn.functional.adaptive_avg_pool2d(x, (1, 1))
            feature_dim_fc = torch.flatten(x, 1).shape[1]
...
class ModernConvNeck(nn.Module):
    """
    EfficientNetV2 후단에 붙일 lightweight modern conv neck
    ConvNeXt V2 아이디어 참고:
    DW large-kernel conv -> LN -> channel MLP -> GELU -> GRN -> projection -> residual
    """
    def __init__(self, channels, kernel_size=7, expansion=4, drop=0.0):
        super().__init__()
...
class EfficientNetV2ForImageClassification_v3(nn.Module):
    """EfficientNetV2 with a plug-in P-SPGA block.

    Flow: stem -> blocks -> P-SPGA -> bottleneck -> avgpool -> classifier.
    External CBAM/SAFE modules are intentionally removed to isolate the P-SPGA effect.
    """

    def __init__(self, num_labels=7, img_size=224, patch_size=16, hidden_dim=512,
                 model_variant='s', dilation=3, stat_type='mean_std_range',
                 use_branch=True, branch_type='full', use_stat_gate=True, use_prior_map=True,
                 use_learnable_attention=True, use_progressive_fusion=True,
                 residual_scale=1.0):
        super(EfficientNetV2ForImageClassification_v3, self).__init__()
        ...
        self.pspga_block = PSPGABlock(
            ...
            use_stat_gate=use_stat_gate,
            use_prior_map=use_prior_map,
            use_learnable_attention=use_learnable_attention,
            use_progressive_fusion=use_progressive_fusion,
    ...
    def forward(self, images, labels=None, epoch=None):
        ...
        x = self.backbone.head.bottleneck(x)
        x = self.backbone.head.avgpool(x)
        features = self.backbone.head.flatten(x)
        logits = self.classifier(features)
